...
def fetch_oi(current_strike,expiry_dt,dajs,look_back):
    ...
    temp=f"value of call open interest change is {ce_sum[-1]} and put open interest change is {pe_sum[-1]}"
...
def get_expiry_date():
    ...
    df=df.sort_values()
    df[0]=df[0].replace(hour=15,minute=31)
...
def banknifty_oi ():
    ...
    label1 = tk.Label(root, text= a, fg='green', font=('helvetica', 12, 'bold'))
    #label2 = tk.Label(root, text= b, fg='green', font=('helvetica', 12, 'bold'))
    canvas1.create_window(150, 200, window=label1)
    #canvas1.create_window(150, 200, window=label2)
def expiry_date_button():
    a=str(get_expiry_date())
    label1=tk.Label(root,text=a, fg='green', font=('helvetica', 12, 'bold'))
    canvas1.create_window(150, 200, window=label1)
...
if __name__=="__main__":
    ...
    canvas1.pack()
    button1 = tk.Button(text='Banknifty OI',command=banknifty_oi, bg='brown',fg='white')
    button2 = tk.Button(text='get expiry_date',command=expiry_date_button, bg='brown',fg='white')
